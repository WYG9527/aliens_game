...
class Ship():
	def __init__ (self,ai_settings,screen):						#screen将指定要将飞船绘制到什么地方。ai_settings为Settings的一个实例。
		...
		# 加载飞船图像并获取其外接矩形
		original_image = pygame.image.load('images/ship.png')	#为加载图像，我们调用了pygame.image.load()。这个函数返回一个表示飞船的surface，而我们将这个surface存储到了self.image中。
		'''该函数从指定路径加载图像文件（如 PNG、JPG、BMP 等），将其转换为 Pygame 的 Surface 对象。Surface 对象是 Pygame 中表示图像的基础数据结构，支持后续的绘制、变换等操作'''
		
		new_width=original_image.get_width()//4					#在 Pygame 中缩小飞船图片的核心方法是使用 pygame.transform.scale() 或 pygame.transform.smoothscale()
